          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            {teamMembers
              ...
              .map((otherMember) => (
                <Card
                  key={otherMember.id}
                  className="group/other gap-0 p-0 hover:glow-ring focus-within:glow-ring"
                >
                  <Link href={`/team/${otherMember.id}`} className="block rounded-xl">
                    <div className="relative aspect-[16/10] overflow-hidden">
                      <Image
                        src={otherMember.image}
                        alt={otherMember.name}
                        fill
                        className="object-cover transition-transform duration-700 ease-out group-hover/other:scale-[1.03]"
